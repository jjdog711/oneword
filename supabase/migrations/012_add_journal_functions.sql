-- Add SQL functions to bypass schema cache issues for journal entries
...
AS $$
BEGIN
    -- Check if user has already added a journal entry today
    IF EXISTS (
...
BEGIN
    ...
    ORDER BY je.date_local DESC;
END;
$$;

-- Function to get today's journal entry
CREATE OR REPLACE FUNCTION get_todays_journal_entry(p_user_id UUID)
RETURNS TABLE (
    id UUID,
    user_id UUID,
    date_local DATE,
...
BEGIN
    ...
    FROM journal_entries je
    WHERE je.user_id = p_user_id 
    AND je.date_local = CURRENT_DATE;
END;
$$;
